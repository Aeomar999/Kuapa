...
describe('PaymentsService', () => {
  ...
  beforeEach(() => {
    ...
    service = new PaymentsService(prisma as any, config as any, logger as any);
  });
  ...
  it('should initialize payment and return authorization url', async () => {
    prisma.order.findFirst.mockResolvedValue({ id: 'o1', paymentStatus: 'pending', total: 100 } as any);
  ...
  it('should verify payment', async () => {
    jest.spyOn(service as any, 'paystackGet').mockResolvedValue({
      status: true,
      data: { status: 'success', reference: 'ref-1', channel: 'card', amount: 10000, currency: 'GHS' },
    });
    prisma.order.findFirst.mockResolvedValue({ id: 'o1', paystackRef: 'ref-1', userId: 'u1' } as any);
  ...
  it('should handle webhook', async () => {
    ...
    const result = await service.handleWebhook({ event: 'charge.success', data: { reference: 'ref-1' } });
