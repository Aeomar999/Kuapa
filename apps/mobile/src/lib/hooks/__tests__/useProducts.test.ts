...
describe("useProducts", () => {
  ...
  it("should fetch all products on mount", async () => {
    (productsApi.getProducts as jest.Mock).mockResolvedValue({ data: { data: [{ id: "1", name: "Test Product" }] } });
  ...
  it("should fetch with category filter", async () => {
    (productsApi.getProducts as jest.Mock).mockResolvedValue({ data: { data: [{ id: "2", name: "Electronics", category: "electronics" }] } });
    const { result} = renderHook(() => useProducts({ category: "electronics" }), { wrapper: createWrapper() });
    await waitFor(() => expect(result.current.isPending).toBeFalsy());
    expect(productsApi.getProducts).toHaveBeenCalledWith({ category: "electronics" });
    expect(result.current.data).toEqual([{ id: "2", name: "Electronics", category: "electronics" }]);
  });
  ...
  it("should fetch product by id", async () => {
    (productsApi.getProduct as jest.Mock).mockResolvedValue({ data: { id: "1", name: "Single Product" } });
...
describe("useCategories", () => {
  ...
  it("should fetch categories on mount", async () => {
    (productsApi.getCategories as jest.Mock).mockResolvedValue({ data: [{ id: "1", name: "Electronics" }] });
